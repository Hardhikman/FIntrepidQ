...
---
# Preview (shows warning, doesn't delete)
python db_fileops/db_maintenance.py purge-all

# Actually delete all reports
...
Clean up reports based on date criteria using the cleanup-by-date command.

### Commands

| Command | Description |
|---------|-------------|
| `cleanup-by-date --before YYYY-MM-DD` | Delete reports created before specified date |
| `cleanup-by-date --older-than N` | Delete reports older than N days |
| `cleanup-by-date --ticker SYMBOL` | Filter cleanup to specific ticker |
| `cleanup-by-date --dry-run` | Preview without deleting |

### Examples

```bash
# Preview reports before Dec 8 (dry run)
python db_fileops/db_maintenance.py cleanup-by-date --before 2025-12-08 --dry-run

# Delete reports older than 7 days
python db_fileops/db_maintenance.py cleanup-by-date --older-than 7

# Delete only TSLA reports before Dec 8
python db_fileops/db_maintenance.py cleanup-by-date --before 2025-12-08 --ticker TSLA

# Preview TSLA reports older than 5 days
python db_fileops/db_maintenance.py cleanup-by-date --older-than 5 --ticker TSLA --dry-run
```

### Notes

- Reports with less than 2000 characters are flagged as potentially incomplete
- Always use `--dry-run` first to preview what will be deleted
- Date format must be YYYY-MM-DD (e.g., 2025-12-08)

---

## File Cleanup

Manage markdown report files in the `reports/` directory.

### Automatic File Cleanup

When database cleanup runs (via `save_analysis_to_memory()` or manual cleanup), old report files are also automatically deleted to keep the `reports/` folder in sync with the database.

### Manual File Cleanup

```bash
# Preview file cleanup (dry run)
python db_fileops/db_maintenance.py cleanup-files --dry-run

# Cleanup all tickers, keep latest 3 files each
python db_fileops/db_maintenance.py cleanup-files

# Cleanup specific ticker
python db_fileops/db_maintenance.py cleanup-files --ticker AAPL

# Keep only 1 file per ticker
python db_fileops/db_maintenance.py cleanup-files --keep 1
```
...
| Command | Description |
|---------|-------------|
| `cleanup-files` | Cleanup old report files |
| `cleanup-files --ticker SYMBOL` | Cleanup only specific ticker |
| `cleanup-files --keep N` | Keep latest N files per ticker (default: 3) |
| `cleanup-files --dry-run` | Preview without deleting |
